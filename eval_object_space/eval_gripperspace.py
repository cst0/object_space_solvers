...
from env_block import BlockStackingEnv
...
starttime = time.time()
def train_and_eval(model, env, eval_every=1000, total_learning_time=1000000, current_iter=0, df=None):
    if df is None:
        df = pd.DataFrame()
    # Train the agent
    ...
    for step in tqdm(range(n_steps)):
        ...
        df.to_csv(f"block_stack_ppo_standard_{starttime}.csv")
