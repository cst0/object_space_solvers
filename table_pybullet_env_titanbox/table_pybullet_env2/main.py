...
class ArmSpaceEnv(gym.Env):
    ...
    def get_reward(self):
        ...
        distance_reward = -np.linalg.norm(cube_position - self.goal_xyz)
        return -1  # TODO
...
class TrainOnPlaceCube:
    def __init__(
        ...
        #self.env = gym.make("gymnasium:ArmSpaceEnv-v0", apply_api_compatibility=True)
        # self.env = make_vec_env(lambda: self.env, n_envs=1)
        self.env = ArmSpaceEnv()
...
#            obs, rewards, terminateds, dones, info = self.env.step(action)
#
#
#class RunEnv:
#    def __init__(self):
...
if __name__ == "__main__":
    #ArmSpaceEnv().test_movements()
    _id = os.environ.get("ID", 0)
    print("=== Training agent ", _id, " ===")
    train = TrainOnPlaceCube(
        f"policies/ppo_action_space_iter_{_id}",
